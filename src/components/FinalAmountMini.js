export default function FinalAmountMini({ totalCaseCost, surplusMonthlyIncome, creditors, calculateCaseCostWithInterest, monthlyIncome, monthlyExpense }) {
    ...
    return (
      <div className="bg-slate-200 text-black py-1 px-4 my-4 rounded-md w-full m-auto text-xl font-light">
        ...
        <div className="flex w-full justify-between text-center mt-12">
          <div className="w-1/2">Creditor:</div>
          <div className="w-1/2">per måned:</div>
        ...
        <div className="my-4 text-center mb-20">
          {creditors.map((creditor, creditorIndex) => {
            ...
            <div key={creditorIndex} className="flex flex-col justify-between ">
              <div className="flex justify-center w-full border-b-2 border-gray-400">
                <div className="w-1/2 border border-gray-400">{creditor.creditorName}</div>
                <div className="w-1/2 border border-gray-400">{Math.floor(creditor.cases.reduce((total, currentCase) => total + currentCase.caseCost, 0) / totalCaseCost * surplusMonthlyIncome)},-</div>
              </div>
              {creditor.cases.map((caseItem, caseIndex) => (
                <div key={caseIndex} className="flex justify-center w-full">
                  <div className="w-1/2 border border-gray-400" style={{ paddingLeft: '20px' }}>{`Case ${caseIndex + 1}`}</div>
                  <div className="w-1/2 border border-gray-400">{caseItem.caseCost / totalCaseCost * surplusMonthlyIncome},-</div>
                </div>
              ))}
            </div>
            )
